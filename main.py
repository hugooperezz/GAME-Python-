...
#Metodo que inicia el programa preguntandole al usuario como quiere operar
def inicio():
    print("\n======== Como quieres operar ========\n" + "-" * 22 ,"\n1. Consola\n" + "-" * 22 , "\n2. Interfaz\n" + "-" * 22,"\n======================")
...
#Metodo que crea una sub ventana que elimina el producto introducido por la entrada con la ayuda del metodo eliminar producto
...
#Metodo que muy parecido al de mostrar inventario con la unica diferencia es que lo imprime de mayor a menor por precio
def productoCaroBarato_interfaz():
    productoCaroBarato = sorted(productos, key=lambda x: x[1], reverse=True)
    # Construcción de la información con un formato alineado
    informacion = "-" * 40 + "\n"
    for i in productoCaroBarato:
...
def resumen_Interfaz(nueva_ventana):
    ...
    nueva_ventana.geometry("350x300") 

    # Frame para centrar contenido
    frame = tk.Frame(nueva_ventana)
    frame.pack(expand=True, padx=20, pady=20)

    Titulo = tk.Label(frame, text="Resumen Inventario", font=("Courier", 16, "bold"), fg="blue")
    Titulo.grid(row=0, column=0, columnspan=3, pady=10, sticky="nsew")

    entrada1 = tk.Label(frame, text=f"Total de productos en la tienda: {totalProductos()}", font=("Arial", 12))
    entrada1.grid(row=1, column=0, columnspan=3, pady=5, sticky="nsew")

    entrada2 = tk.Label(frame, text=productoCaroBarato_interfaz(), font=("Arial", 12))
    entrada2.grid(row=2, column=0, columnspan=3, pady=5, sticky="nsew")

    entrada3 = tk.Label(frame, text=f"Valor total de los productos: {valorTotal()} €", font=("Arial", 12))
    entrada3.grid(row=3, column=0, columnspan=3, pady=5, sticky="nsew")     
...
#Metodo que le da la funcionalidad al boton de salir que destruye la ventana principal
def salir_Interfaz(ventana):
    ventana.destroy()

#Metodo que crea la ventana principal el cual incluye toda su configuracion
def interfaz():
    # Crear la ventana
    ventana = tk.Tk()
